...
I was given a dataset to work with that contained information on the activity of 30 FitBit users. I found the heartrate table very interesting as it displayed user heartrate for every second over the timeframe of the study. 
...
The dataset is downloaded as a file named ‘Fitbase Data 4.12.16 – 5.12.16’. The files within are .csv files. Most of the data is stored in long format.

To determine the credibility of the data I looked at 5 qualities: reliability, originality, comprehensiveness, how up to date it is, and citability. 
...
The data is collected firsthand and therefore I am confident in its originality. It aslo includes all relevant activities we are looking for across different time frames (weight, steps, sleep minutes etc.). 

This data was collected in 2016 however the data is updated annually. The last update was 2 years ago. 

This dataset is licensed under CC0, meaning it is public domain. This allows us to manipulate and reproduce the data without copy right concerns.
...
I then changed the schema, separated the date and time, and hid the first row since it displayed the column names.
...
Next, I checked different columns for any null data using the IS NULL function. Lastly, I made sure the dates were inputted correctly using the LENGTH(STRING) function to check if any data didn’t meet the 10-string requirement.
...
We started by looking to plot the heartrate data without changing anything. This would give us an idea of how each user’s heartrate fluctuated over the course of the study.
...
I plotted the heartrate of each individual user vs the date (day). I used a boxplot and whisker chart to visualize the median, interquartile range, maximum, and minimum values during each day of the study. This plot helps us an visualize the resting heartrate and note when the heartrate has gone over/under dangerous levels. 
...
The visualization helps us see that some users in the study show potentially concerning heartrate levels. User ID 4388161847 (second from the top) recorded a median heartrate that was consistently close to the 60bmp lower limit. This means the lower hinge and lower whisker were constantly below 60bpm.
